---
title: "Complex Social Systems: Housing"
collection: talks
type: "Conference Presentation"
permalink: /talks/2025-lcc-complex-social-systems-housing
venue: "Life Course Centre Conference (ARC Centre of Excellence for Children and Families over the Life Course)"
date: 2025-07-01
location: "Brisbane, Australia"
---

Presented on housing as a complex social system and how agent-based modelling (ABM) can be used to understand and inform housing policy.

I began by discussing what makes social systems complex — multiple interacting inputs, non-linear dynamics, feedback loops, contested evidence, and the modeller's own role in data collection and analysis. I then framed housing as a complex social system in its own right: one shaped by multiple interacting agents (households, developers, financial institutions and governments), heterogeneity and inequality, feedback loops, and emergent phenomena such as bubbles, segregation, gentrification and affordability crises.

Drawing on the idea that good models are, in Picasso's words, "lies that help us see the truth", I argued that computational models are valuable because they force us to make our implicit assumptions explicit, stripping out noise to reveal the most important dynamics.

I shared findings from a literature survey of 27 agent-based housing models across 11 countries, reviewed along dimensions including model objectives, agent behaviour, market mechanisms, spatial scope, policies studied, calibration and validation. I highlighted key gaps — renters and informal housing are often neglected, Global South contexts are under-represented, and there is a need for better empirical validation and interdisciplinary models.

Finally, I discussed ongoing work in the Australian housing context, modelling affordable and social housing supply for low-income renters, and a participatory approach in which key stakeholders help shape model assumptions, data needs, and the policy levers worth simulating — from investment incentives and rental assistance to build-to-rent development and short-term rentals — bridging the path from modelling to policy.
